  ...
  fs.createReadStream(req.file.path)
    ...
    .on("data", async (data) => {
      ...
      const cleanedData = {
        ...
        isFSale:
          data.isFSale !== undefined ? data.isFSale === "true" : undefined, // Kiểm tra isFSale, nếu không có thì để undefined để model dùng mặc định
      };
      ...
      if (cleanedData.title) {
        ...
        if (!categoryDoc) {
          console.log(
            `Category not found. Creating new category: ${cleanedData.genres}`
          );
        ...
        if (!authorDoc) {
          console.log(
            `Author not found. Creating new author: ${cleanedData.author}`
          );
        ...
        try {
          ...
          if (categoryDoc) {
            ...
            console.log(
              `Incremented ctnBook for category: ${categoryDoc.name}`
            );
          }
...
router.post("/", async (req, res) => {
  try {
    ...
    if (!categoryDoc) {
      console.log(
        `Category not found. Creating new category: ${req.body.genres}`
      );
    ...
    if (!book) {
      ...
      return res
        .status(500)
        .json({ success: false, message: "The book cannot be created!" });
    }
    ...
    await Authors.updateOne({ _id: authorDoc._id }, { $inc: { ctnBook: 1 } });
  ...
  try {
    ...
    if (
      ...
      const discountPercent =
        req.body.discountPercent || currentBook.discountPercent;
    ...
    if (oldGenres !== newGenres) {
      ...
      await Category.updateOne({ name: oldGenres }, { $inc: { ctnBook: -1 } });
    ...
    if (oldAuthor !== newAuthor) {
      ...
      await Authors.updateOne({ name: oldAuthor }, { $inc: { ctnBook: -1 } });
